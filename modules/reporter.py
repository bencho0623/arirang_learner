...
<body>
  <div class="container">
    <section class="panel hero">
      ...
      <div class="header-row">
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <span class="badge" id="bookmark-count-badge">遺곷쭏??0媛?/span>
        </div>
    ...
    <section class="panel">
      ...
      <div class="script-tools">
        <div class="script-hint">?ㅽ겕濡ㅻ줈 ?꾩껜 ?뺤씤 媛??쨌 ?꾩슂 ???꾩껜蹂닿린</div>
        <button id="script-toggle" class="script-toggle" type="button">?꾩껜蹂닿린</button>
      </div>
  ...
  <script>
    ...
    function renderHeader() {{
      ...
      document.getElementById("vocab-count-badge").textContent = "?댄쐶 " + vocabData.length + "媛?;
    ...
    function renderScript() {{
      ...
      toggle.addEventListener("click", () => {{
        ...
        toggle.textContent = box.classList.contains("expanded") ? "?묎린" : "?꾩껜蹂닿린";
      }});
    }}
    ...
    function renderCards() {{
      ...
      grid.innerHTML = vocabData.map((v) => {{
        ...
        const ko = String(v.translation_ko || "").trim() || "?ъ쟾 誘몃벑濡?;
        ...
        return "<article class='v-card " + bookmarked + "' id='card-" + lemma + "'>" +
               "<button class='bookmark-btn' data-word='" + escHtml(v.word) + "' aria-pressed='" + pressed + "'>?뵔</button>" +
               "<div class='card-top'>" +
               "<span class='word'>" + escHtml(v.word || "") + "</span>" +
               "<span class='cefr'>" + escHtml(v.cefr_level || "") + "</span>" +
               "<span class='phonetic'>" + escHtml(v.phonetic || "") + "</span>" +
               "</div>" +
               "<div class='sub'>" + escHtml(v.pos_ko || "") + " 쨌 鍮덈룄?먯닔 " + escHtml(v.frequency_score) + "</div>" +
               "<div class='card-body'>" +
    ...
    function updateBookmarkCount() {{
      ...
      document.getElementById("bookmark-count-badge").textContent = "遺곷쭏??" + n + "媛?;
    }}
        ...
        qEl.querySelectorAll(".choice").forEach((btn) => {{
          btn.addEventListener("click", () => {{
            ...
            if (picked === q.answer) {{
              ...
              result.textContent = "?뺣떟! " + (q.definition ? q.definition : "");
            }} else {{
              btn.classList.add("wrong");
              result.textContent = "?ㅻ떟. ?뺣떟: " + q.answer + (q.definition ? " | " + q.definition : "");
            }}
            refresh();
          }});
        }});
